...
resource "aws_cloudwatch_log_group" "cloudwatch" {
  count             = var.enabled && var.elasticsearch_enabled && var.enable_logs ? 1 : 0
...
resource "aws_iam_service_linked_role" "default" {
  count            = var.enabled && var.elasticsearch_enabled && var.enable_iam_service_linked_role ? 1 : 0
...
resource "aws_iam_role" "default" {
  count              = var.enabled && var.elasticsearch_enabled ? 1 : 0
...
module "cognito-role" {
  ...
  environment = var.environment
  label_order = ["name"]
  enabled     = var.elasticsearch_enabled && var.cognito_enabled
...
module "es_dns" {
  ...
  version        = "1.0.1"
  record_enabled = var.elasticsearch_enabled && var.dns_enabled
...
################################################################################
# OpenSearch Module added by user
################################################################################

data "aws_region" "current" {
  count = var.enabled && var.opensearch_enabled ? 1 : 0
}
  ...
  dynamic "off_peak_window_options" {
    ...
    content {
      ...
      dynamic "off_peak_window" {
        for_each = try([off_peak_window.value.off_peak_window], [])
...
resource "aws_opensearch_package_association" "this" {
  for_each = { for k, v in var.package_associations : k => v if var.enabled && var.opensearch_enabled }
...
resource "aws_opensearch_vpc_endpoint" "this" {
  for_each = { for k, v in var.vpc_endpoints : k => v if var.enabled && var.opensearch_enabled }
...
locals {
  create_access_policy = var.enabled && var.opensearch_enabled && var.create_access_policy && (length(var.access_policy_statements) > 0 || length(var.access_policy_source_policy_documents) > 0 || length(var.access_policy_override_policy_documents) > 0)
}

resource "aws_opensearch_domain_policy" "this" {
  count = var.enabled && var.opensearch_enabled && var.enable_access_policy && (local.create_access_policy || var.access_policies != null) ? 1 : 0
...
resource "aws_opensearch_domain_saml_options" "this" {
  count = var.enabled && var.opensearch_enabled && var.create_saml_options ? 1 : 0
...
resource "aws_opensearch_outbound_connection" "this" {
  for_each = { for k, v in var.outbound_connections : k => v if var.enabled && var.opensearch_enabled }
...
locals {
  create_cloudwatch_log_groups = var.enabled && var.opensearch_enabled && var.create_cloudwatch_log_groups
}
...
locals {
  create_security_group = var.enabled && var.opensearch_enabled && var.create_security_group && length(var.vpc_options) > 0
